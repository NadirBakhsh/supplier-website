...
function AppleIcon() {
  return (
    <svg viewBox="0 0 384 512" className="h-5 w-5 fill-current sm:h-6 sm:w-6" aria-hidden="true">
...
function PlayIcon() {
  return (
    <svg viewBox="0 0 512 512" className="h-5 w-5 fill-current sm:h-6 sm:w-6" aria-hidden="true">
...
function StoreButton({ href, icon, eyebrow, label, tone }: StoreButtonProps) {
  ...
  const classes = cn(
    "group flex min-w-0 items-center gap-2 rounded-xl border px-2.5 py-2 transition-colors sm:gap-3 sm:px-4 sm:py-2.5",
    tone === "dark"
  ...
  const content = (
    <>
      ...
      <span className="flex min-w-0 flex-col items-start leading-tight sm:flex-col">
        <span
          className={cn(
  ...
  if (isDisabled) {
    return (
      <Tooltip>
        <TooltipTrigger type="button" className={classes} aria-disabled="true">
          {content}
        </TooltipTrigger>
...
export function DownloadButtons({ tone = "light", className }: DownloadButtonsProps) {
  return (
    <div
      className={cn(
        "flex flex-nowrap items-stretch gap-2 *:min-w-0 *:flex-1 sm:flex-wrap sm:gap-3 sm:*:flex-none",
        className
      )}
    >
      <StoreButton
